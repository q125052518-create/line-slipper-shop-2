      ...
      <article class="order ${hasCancelRequest ? "has-cancel-request" : ""}">
        ...
        <ul>
          ${(order.items || []).map((item) => `
            <li>${escapeHtml(item.orderTypeLabel || (item.orderType === "box" ? "整箱訂購" : "散貨訂購"))} / ${escapeHtml(item.marketName)} / ${escapeHtml(item.productName)} / ${escapeHtml(item.variantName)} / ${escapeHtml(item.barcode)} x ${item.quantity}，${formatMoney(item.subtotal)}</li>
          `).join("")}
        </ul>
